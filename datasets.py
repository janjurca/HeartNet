...
class GomezT1(Dataset):
    def __init__(self, root, portion=0.75):
        ...
        for file in files:
            ...
            try:
                with open(f"{root}/annotated/{image_id}/GT.pickle") as fp:
                    GT = pickle.load(fp)
            except FileNotFoundError:
                GT = self.generateGT(root, image_id, image)

            self.data.append(
                (
                    torch.tensor([image.ct_scan[(depth//2)-32:(depth//2)+32, (width//2)-64:(width//2)+64, (height//2)-64:(height//2)+64]]),
                    torch.tensor([GT[(depth//2)-32:(depth//2)+32, (width//2)-64:(width//2)+64, (height//2)-64:(height//2)+64]]),
                )
            )
